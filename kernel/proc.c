...
void schedulerCFS(void);
void schedulerPriority(void);
void scheduler_old(void);
void set_police(int new_policy);
...
found:
  ...
  // task 5 - set the accumulator to the minimum 
...
int wait(uint64 addr, uint64 addr2)
{
  struct proc *pp;
  int havekids, pid;
  struct proc *p = myproc();
